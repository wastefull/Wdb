...
export async function updateEvidenceValidation(c: any) {
  logger.log("updateEvidenceValidation handler called!");
  try {
    ...
    if (!["pending", "validated", "flagged", "duplicate"].includes(status)) {
      return c.json(
        {
          error:
            "Invalid validation status. Must be: pending, validated, flagged, or duplicate",
        },
        400
      );
    }
    ...
    logger.log(
      `✓ Updated validation status for evidence point ${id} to ${status}`
    );
    ...
    logger.error("Error updating evidence validation:", error);
    return c.json(
      {
...
export async function createAggregation(c: any) {
  try {
    ...
    logger.log(
      `✓ Created aggregation ${newAggregation.id} for material ${newAggregation.material_id}, parameter ${newAggregation.parameter}`
    );
...
export async function getAggregationsByMaterial(c: any) {
  try {
    ...
    const currentAggregations =
      await aggregations.getCurrentAggregationsByMaterial(materialId, options);
    ...
    logger.error("Error fetching aggregations:", error);
    return c.json(
      {
...
export async function getAggregationHistory(c: any) {
  try {
    ...
    const history = await aggregations.getAggregationHistory(
      materialId,
      parameter
    );
    ...
    logger.error("Error fetching aggregation history:", error);
    return c.json(
      {
...
export async function getAggregationStats(c: any) {
  try {
    ...
    logger.error("Error fetching aggregation stats:", error);
    return c.json(
      {
...
export async function checkSourceCanDelete(c: any) {
  try {
    ...
    logger.error("Error checking source deletion:", error);
    return c.json(
      {

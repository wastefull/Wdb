  ...
  return [
    {
      ...
      description:
        "Verify DOI format normalization across different input formats",
      phase: "9.0.5",
      category: "Sources",
      testFn: async () => {
        try {
          ...
          const response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/sources/normalize-doi`,
            {
              method: "POST",
              headers: {
          ...
          if (!response.ok) {
            ...
            return {
              success: false,
              message: data.error || "Failed to normalize DOIs",
            };
          }
    ...
    },
    {
      ...
      testFn: async () => {
        try {
          const response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/sources/check-duplicate`,
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${publicAnonKey}`,
        ...
        try {
          ...
          return {
            ...
            message: result.isDuplicate
              ? `Similar title found Match: "${result.existingSource?.title}" (${result.similarity}% similar)`

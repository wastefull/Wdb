  ...
  return [
    {
      ...
      testFn: async () => {
        if (!user) {
          return {
            success: false,
            message: "Must be authenticated to create audit logs",
          };
        }
        ...
        try {
          ...
          if (!response.ok) {
            ...
            return {
              success: false,
              message: data.error || "Failed to create audit log",
            };
          }
          ...
          return {
            success: true,
            message: `Audit log created ✓ (ID: ${data.auditId.slice(
              0,
              30
            )}...)`,
          };
    ...
    },
    {
      ...
      testFn: async () => {
        if (!user) {
          return {
            success: false,
            message: "Must be authenticated to fetch audit logs",
          };
        }
        ...
        try {
          ...
          if (!response.ok) {
            ...
            return {
              success: false,
              message: data.error || "Failed to fetch audit logs",
            };
          }
    ...
    },
    {
      ...
      testFn: async () => {
        if (!user) {
          return {
            success: false,
            message: "Must be authenticated to get audit statistics",
          };
        }
        ...
        try {
          ...
          if (!response.ok) {
            ...
            return {
              success: false,
              message: data.error || "Failed to get audit stats",
            };
          }
          ...
          return {
            success: true,
            message: `Stats retrieved ✓ (${data.stats.total} total events, ${
              Object.keys(data.stats.byUser).length
            } users)`,
          };
        } catch (error) {
          return {
    ...
    },
    {
      ...
      testFn: async () => {
        if (!user) {
          return {
            success: false,
            message: "Must be authenticated to get audit logs",
          };
        }
        ...
        try {
          ...
          if (!listData.logs || listData.logs.length === 0) {
            return {
              success: false,
              message:
                "No audit logs found. Create one first (Test: Create Audit Log)",
            };
          }
          ...
          const response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/audit/logs/${testId}`,
            {
              headers: getAuthHeaders(accessToken),
            }
          );
          ...
          if (!response.ok) {
            ...
            return {
              success: false,
              message: data.error || "Failed to retrieve audit log",
            };
          }
          ...
          if (!data.log || data.log.id !== testId) {
            return {
              success: false,
              message: "Retrieved audit log does not match requested ID",
            };
          }
      ...
      testFn: async () => {
        ...
        try {
          ...
          const page1Response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/audit/logs?limit=5&offset=0`,
            {
              headers: getAuthHeaders(accessToken),
            }
          );
          ...
          const page2Response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/audit/logs?limit=5&offset=5`,
            {
              headers: getAuthHeaders(accessToken),
            }
          );
          ...
          if (page1Data.logs.length > 0 && page2Data.logs.length > 0) {
            ...
            const hasOverlap = page2Data.logs.some((log: any) =>
              page1Ids.has(log.id)
            );
            ...
            if (hasOverlap) {
              return {
                success: false,
                message: "Pagination returned overlapping results",
              };
            }
          }
          ...
          return {
            success: false,
            message: `Error testing pagination: ${
              error instanceof Error ? error.message : "Unknown error"
            }`,
          };
        }
    ...
    },
    {
      ...
      testFn: async () => {
        if (!user) {
          return {
            success: false,
            message: "Must be authenticated as admin to create materials",
          };
        }
        ...
        try {
          ...
          if (!createResponse.ok) {
            ...
            return {
              success: false,
              message: data.error || "Failed to create test material",
            };
          }
          ...
          const auditResponse = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/audit/logs?entityType=material&entityId=${testMaterialId}`,
            {
              headers: getAuthHeaders(accessToken),
            }
          );
          ...
          const createLog = auditData.logs?.find(
          ...
          if (!createLog) {
            return {
              success: false,
              message: "No audit log found for material creation",
            };
          }
          ...
          return {
            success: false,
            message: `Error testing CRUD audit: ${
              error instanceof Error ? error.message : "Unknown error"
            }`,
          };
        }
    ...
    },
    {
      ...
      testFn: async () => {
        if (!user) {
          return {
            success: false,
            message: "Must be authenticated as admin to update materials",
          };
        }
        ...
        try {
          ...
          const updateResponse = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/materials/${testMaterial.id}`,
            {
              method: "PUT",
              headers: getAuthHeaders(accessToken),
              body: JSON.stringify({
          ...
          // Check if audit log was created
          const auditResponse = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/audit/logs?entityType=material&entityId=${testMaterial.id}`,
            {
              headers: getAuthHeaders(accessToken),
            }
          );
          ...
          const updateLog = auditData.logs?.find(
          ...
          if (!updateLog) {
            return {
              success: false,
              message: "No audit log found for material update",
            };
          }
          ...
          return {
            success: false,
            message: `Error testing update audit: ${
              error instanceof Error ? error.message : "Unknown error"
            }`,
          };
        }
    ...
    },
    {
      ...
      testFn: async () => {
        if (!user) {
          return {
            success: false,
            message: "Must be authenticated as admin to delete materials",
          };
        }
        ...
        try {
          ...
          if (!createResponse.ok) {
            return {
              success: false,
              message: "Failed to create test material",
            };
          }
          ...
          const deleteLog = auditData.logs?.find(
          ...
          if (!deleteLog) {
            return {
              success: false,
              message: "No audit log found for material deletion",
            };
          }
          ...
          return {
            success: false,
            message: `Error testing delete audit: ${
              error instanceof Error ? error.message : "Unknown error"
            }`,
          };
        }
    ...
    },
    {
      ...
      description:
        "Comprehensive verification that all material operations were logged",
      phase: "9.0.6",
      category: "Audit",
      testFn: async () => {
        if (!user) {
          return {
            success: false,
            message: "Must be authenticated to verify audit logs",
          };
        }
        ...
        try {
          ...
          const response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/audit/logs?entityType=material&limit=10`,
            {
              headers: getAuthHeaders(accessToken),
            }
          );
          ...
          if (!hasCreate && !hasUpdate && !hasDelete) {
            return {
              success: false,
              message:
                "No CRUD audit logs found. Run previous CRUD audit tests first.",
            };
          }
          ...
          return {
            success: false,
            message: `Error verifying audit logs: ${
              error instanceof Error ? error.message : "Unknown error"
            }`,
          };
        }

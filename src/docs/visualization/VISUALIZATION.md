...
- **Narrative Integrity:** The visualization should visually _teach_ what the data means without needing lengthy prose.
...
const overlap =
  CI_prac_upper >= CI_theo_lower && CI_theo_upper >= CI_prac_lower;
...
function renderRecyclabilityViz(data) {
  ...
  const mode = overlap ? "overlap" : gap < 0.1 ? "near-overlap" : "gap";

...
## 🌐 Option A: Remote Supabase (Recommended for Testing)
...
3. Copy these values:
   ...
   - **Service Role Key** (starts with `eyJhbGc...`) ⚠️ Keep secret!
...
**⚠️ IMPORTANT**: Add `.env.local` to your `.gitignore`:
...
## 🏠 Option B: Local Supabase (Full Local Development)
...
1. Sign in as admin
2. Navigate to **Roadmap** → **Phase 9.2** tab
...
├── /contexts/            # React contexts (Auth, Materials, Navigation)
├── /config/              # Test definitions and phase configs
│   └── /tests/
│       ├── /phases/      # Phase-specific test files
│       └── all.ts        # Centralized test aggregation
├── /data/                # Static data (sources, transforms)
├── /docs/                # Documentation
├── /ontologies/          # Ontology definitions (JSON + TS)
├── /styles/              # Global CSS and Tailwind config
├── /supabase/            # Supabase edge functions
│   └── /functions/
│       └── /server/      # Deno backend code
│           ├── index.tsx           # Main server entry
│           ├── kv_store.tsx        # KV utilities (DO NOT EDIT)
│           ├── evidence-routes.tsx # Evidence endpoints
│           └── exports.tsx         # Export endpoints
...
# Navigate to Roadmap → Phase 9.2 → Run Tests

...
export function AssetUploadManager({ accessToken }: AssetUploadManagerProps) {
  ...
  const handleDelete = async (fileName: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-17cae920/assets/${encodeURIComponent(fileName)}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Upload and manage assets (logo, images, etc.) for use in emails and the application
        </CardDescription>
      </CardHeader>
        ...
        {assets.length > 0 && (
          <div className="pt-4 border-t">
            <p className="text-xs text-muted-foreground mb-2">
              💡 Tip: Upload your logo, then copy its URL to use in the magic link email template
            </p>
          </div>
        )}
      </CardContent>

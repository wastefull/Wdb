...
import { ArrowLeft, CheckCircle, XCircle, Edit3, Flag, Package, FileText, Clock, AlertTriangle } from 'lucide-react';
...
interface Submission {
  ...
  status: 'pending_review' | 'approved' | 'rejected' | 'needs_revision' | 'flagged';
  feedback?: string;
  submitted_by: string;
  reviewed_by?: string;
  created_at: string;
...
export function ContentReviewCenter({ onBack, currentUserId }: ContentReviewCenterProps) {
  ...
  const getSubmissionsByStatus = () => {
    switch (activeTab) {
      ...
      case 'pending':
        return submissions.filter(s => s.status === 'needs_revision' || s.status === 'approved');
      case 'moderation':
        return submissions.filter(s => s.status === 'flagged' || s.status === 'rejected');
      default:
        return [];
    }
  };
        ...
        <TabsContent value={activeTab}>
          {loading ? (
            ...
            <div className="text-center py-12">
              <div className="mb-4">
                {activeTab === 'review' && <Clock size={48} className="mx-auto text-black/20 dark:text-white/20" />}
                {activeTab === 'pending' && <AlertTriangle size={48} className="mx-auto text-black/20 dark:text-white/20" />}
                {activeTab === 'moderation' && <Flag size={48} className="mx-auto text-black/20 dark:text-white/20" />}
              </div>
              <p className="font-['Sniglet:Regular',_sans-serif] text-[14px] text-black/70 dark:text-white/70">
...
function SubmissionCard({
  ...
  const getSubmissionSnippet = () => {
    if (submission.type === 'edit_material' && submission.content_data?.change_reason) {
      return submission.content_data.change_reason;
    }
    return submission.content_data?.description || submission.content_data?.content?.substring(0, 100) || 'No description';
  };

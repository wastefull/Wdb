...
const PARAMETER_INFO: Record<string, { name: string; unit: string; dimension: string; description: string }> = {
  // Recyclability
  Y_value: { name: 'Yield (Y)', unit: '%', dimension: 'Recyclability', description: 'Material retention in recycling process' },
  D_value: { name: 'Degradation (D)', unit: '%', dimension: 'Recyclability', description: 'Quality loss per cycle' },
  C_value: { name: 'Contamination (C)', unit: '%', dimension: 'Recyclability', description: 'Tolerance to contaminants' },
  M_value: { name: 'Infrastructure (M)', unit: '%', dimension: 'Recyclability', description: 'Facility access maturity' },
  E_value: { name: 'Energy (E)', unit: '%', dimension: 'Recyclability', description: 'Energy efficiency vs virgin material' },
  
  // Compostability
  B_value: { name: 'Biodegradability (B)', unit: '%', dimension: 'Compostability', description: 'Breakdown completeness' },
  N_value: { name: 'Nutrient (N)', unit: '%', dimension: 'Compostability', description: 'Compost quality/nutrient value' },
  T_value: { name: 'Time (T)', unit: '%', dimension: 'Compostability', description: 'Degradation speed' },
  H_value: { name: 'Harm (H)', unit: '%', dimension: 'Compostability', description: 'Safety (inverse of toxicity)' },
  
  // Reusability
  L_value: { name: 'Longevity (L)', unit: '%', dimension: 'Reusability', description: 'Physical durability over time' },
  R_value: { name: 'Repairability (R)', unit: '%', dimension: 'Reusability', description: 'Ease of repair/refurbishment' },
  U_value: { name: 'Use Cases (U)', unit: '%', dimension: 'Reusability', description: 'Alternative use possibilities' },
  C_RU_value: { name: 'Consumer (C)', unit: '%', dimension: 'Reusability', description: 'User adoption/behavior' },
};
...
export function SourceDataComparison({ onBack, materials }: SourceDataComparisonProps) {
  ...
  const availableParameters = useMemo(() => {
    ...
    const params = Object.keys(PARAMETER_INFO).filter(key => {
      ...
      const matchesDimension = selectedDimension === 'all' || 
  ...
  const comparisonData = useMemo(() => {
    if (!material || !selectedParameter || !material.sources || material.sources.length === 0) {
      return null;
    }
    ...
    const sourceContributions = sources.map((source, idx) => ({
      source,
      weight: weights[idx],
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = `source-comparison-${material.name}-${selectedParameter}-${new Date().toISOString().split('T')[0]}.json`;
      ...
      {selectedMaterial && selectedParameter && !comparisonData && (
        <Alert className="bg-amber-50 dark:bg-amber-900/20 border-amber-300 dark:border-amber-700">
          ...
          <AlertDescription className="text-[11px] text-amber-800 dark:text-amber-200">
            No sources are tagged for this parameter. Sources must explicitly cite which parameters they support.
          </AlertDescription>
        </Alert>
      )}
        ...
        <div className="space-y-6">
          ...
          <Card className="p-4 bg-[#faf9f6] dark:bg-[#1a1918] border-[#211f1c] dark:border-white/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="p-3 bg-white dark:bg-[#2a2825] rounded-lg border border-[#211f1c] dark:border-white/20">
                <p className="text-[9px] text-black/60 dark:text-white/60 mb-1">Weighted Value</p>
                <p className="text-[18px] text-black dark:text-white">
                  {comparisonData.paramValue.toFixed(1)}
                  <span className="text-[11px] ml-1">{PARAMETER_INFO[selectedParameter].unit}</span>
                </p>
              </div>

              <div className="p-3 bg-white dark:bg-[#2a2825] rounded-lg border border-[#211f1c] dark:border-white/20">
                <p className="text-[9px] text-black/60 dark:text-white/60 mb-1">Contributing Sources</p>
                <p className="text-[18px] text-black dark:text-white">
              ...
              <div className="p-3 bg-white dark:bg-[#2a2825] rounded-lg border border-[#211f1c] dark:border-white/20">
                <p className="text-[9px] text-black/60 dark:text-white/60 mb-1">Total Weight</p>
                <p className="text-[18px] text-black dark:text-white">
              ...
              <div className="p-3 bg-white dark:bg-[#2a2825] rounded-lg border border-[#211f1c] dark:border-white/20">
                <p className="text-[9px] text-black/60 dark:text-white/60 mb-1">Dimension</p>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="font-['Sniglet:Regular',_sans-serif] text-[11px] text-right">Weight</TableHead>
                    <TableHead className="font-['Sniglet:Regular',_sans-serif] text-[11px] text-right">Contribution</TableHead>
                    <TableHead className="font-['Sniglet:Regular',_sans-serif] text-[11px]">Links</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    <TableRow key={idx}>
                      ...
                      <TableCell>
                        <Badge 
                          variant="outline" 
                          className={`text-[8px] ${
                            sc.weight >= 1.0 ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300' :
                            sc.weight >= 0.7 ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300' :
                            'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300'
                          }`}
                        >
                          {sc.weight >= 1.0 ? 'Peer-Reviewed' :
                           sc.weight >= 0.9 ? 'Government' :
                           sc.weight >= 0.7 ? 'Industrial' :
                           sc.weight >= 0.6 ? 'NGO' : 'Internal'}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          {sc.source.doi ? (
                            ...
                            <a
                              href={getGoogleScholarUrl(sc.source.title, sc.source.authors)}
                              target="_blank"
                              rel="noopener noreferrer"
          ...
          <Alert className="bg-blue-50 dark:bg-blue-900/20 border-blue-300 dark:border-blue-700">
            ...
            <AlertDescription className="text-[11px] text-blue-800 dark:text-blue-200">
              <strong>How weighting works:</strong> Each source is assigned a weight based on its type 
              (peer-reviewed = 1.0, government = 0.9, industrial = 0.7, etc.). The final parameter value 
              is calculated using weighted averages, where higher-quality sources have more influence. 
              Contribution percentages show each source's relative influence on the final value.
            </AlertDescription>
          </Alert>
        </div>
      )}

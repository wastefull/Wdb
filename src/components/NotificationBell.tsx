  ...
  const loadNotifications = async () => {
    try {
      ...
      if (isAdmin) {
        ...
        // Remove duplicates
        allNotifications = allNotifications.filter((n, i, arr) => 
          arr.findIndex(t => t.id === n.id) === i
        );
      }
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {unreadCount > 0 && (
            <span 
              className="absolute -top-0.5 -right-0.5 h-3 w-3 flex items-center justify-center bg-[#e6beb5] text-black text-[8px] rounded-full font-['Sniglet:Regular',_sans-serif]"
            >
        ...
        <ScrollArea className="h-[400px]">
          {loading ? (
            ...
            <div className="p-8 text-center">
              <Bell size={32} className="mx-auto mb-3 text-black/30 dark:text-white/30" />

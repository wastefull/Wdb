...
import { ArrowLeft, AlertCircle, CheckCircle, XCircle, TrendingUp, BarChart3, Filter, Download } from 'lucide-react';
...
interface Material {
  ...
  reusability: number;
  // Raw parameter values (these would come from evidence points in Phase 9.1)
  rawParams?: {
    Y?: number;  // Years to degrade
    D?: number;  // Degradability
    C?: number;  // Compostability
    M?: number;  // Methane production
    E?: number;  // Ecotoxicity
    B?: number;  // Biodegradability
    N?: number;  // Novelty
    T?: number;  // Toxicity
    H?: number;  // Human health impact
    L?: number;  // Leachate potential
    R?: number;  // Recyclability
    U?: number;  // Reusability
    C_RU?: number; // Combined R+U
  };
}
...
export function TransformFormulaTesting({ onBack, materials }: TransformFormulaTestingProps) {
  const [selectedParameter, setSelectedParameter] = useState<string>('all');
  const [showOnlyErrors, setShowOnlyErrors] = useState(false);
  const [transformsData, setTransformsData] = useState<TransformsData | null>(null);
  ...
  const materialsWithMockRawParams: Material[] = useMemo(() => {
    return materials.map(material => ({
      ...material,
      rawParams: {
        Y: material.compostability <= 30 ? 500 : material.compostability <= 60 ? 100 : 10,
  ...
  const applyTransform = (parameter: string, rawValue: number): number | null => {
    const transform = transformsData?.transforms.find(t => t.parameter === parameter);
    ...
    try {
      ...
      if (formula.includes('log10')) {
        const match = formula.match(/100\s*-\s*\(10\s*\*\s*log10\((\w+)\s*\+\s*1\)\)/);
        if (match) {
  ...
  const testResults: TestResult[] = useMemo(() => {
    ...
    materialsWithMockRawParams.forEach(material => {
      transformsData?.transforms.forEach(transform => {
        const rawValue = material.rawParams?.[transform.parameter as keyof typeof material.rawParams];
        
        let actualScore: number | null = null;
        if (transform.parameter === 'C') actualScore = material.compostability;
        else if (transform.parameter === 'R') actualScore = material.recyclability;
        else if (transform.parameter === 'U') actualScore = material.reusability;
        else if (transform.parameter === 'C_RU') actualScore = (material.recyclability + material.reusability) / 2;

        const computedScore = rawValue != null ? applyTransform(transform.parameter, rawValue) : null;
        const difference = computedScore != null && actualScore != null 
  ...
  const statistics = useMemo(() => {
    const paramStats: { [key: string]: { total: number; errors: number; avgDiff: number; maxDiff: number } } = {};

    transformsData?.transforms.forEach(transform => {
      const paramResults = testResults.filter(r => r.parameter === transform.parameter);
      const errors = paramResults.filter(r => r.error !== null).length;
      const diffs = paramResults.filter(r => r.difference != null).map(r => r.difference!);
  ...
  const getParameterColor = (param: string) => {
    const colors: { [key: string]: string } = {
      Y: '#e6beb5', D: '#b8c8cb', C: '#a8d5ba', M: '#f4d5a6',
      E: '#d4a5a5', B: '#c4b5d5', N: '#f5e6d3', T: '#e5c3c6',
      H: '#d4e4f7', L: '#d5e8d4', R: '#fff2cc', U: '#ffe6cc', C_RU: '#e1d5e7'
    };
  ...
  return (
    <div className="h-full flex flex-col bg-[#e5e4dc] dark:bg-[#1a1917]">
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="p-4 rounded-lg bg-[#e5e4dc] dark:bg-[#1a1917] border border-[#211f1c]/20 dark:border-white/20">
            <div className="flex items-center gap-2 mb-2">
              <BarChart3 size={16} className="text-black/60 dark:text-white/60" />
          ...
          <div className="p-4 rounded-lg bg-[#e5e4dc] dark:bg-[#1a1917] border border-[#211f1c]/20 dark:border-white/20">
            <div className="flex items-center gap-2 mb-2">
              <TrendingUp size={16} className="text-black/60 dark:text-white/60" />
              <span className="font-['Sniglet'] text-[11px] text-black/60 dark:text-white/60">
                Success Rate
              </span>
            </div>
      ...
      <div className="p-6 bg-white dark:bg-[#2a2825] border-b border-[#211f1c]/20 dark:border-white/20">
        <h3 className="font-['Fredoka_One'] text-[16px] text-black dark:text-white mb-4">
          Parameter Statistics
        </h3>
        <div className="grid grid-cols-7 gap-3">
          {transformsData?.transforms.map(transform => {
            const stats = statistics[transform.parameter];
            const successRate = stats ? ((stats.total - stats.errors) / stats.total) * 100 : 0;
            ...
            return (
              <button
                ...
                style={{ backgroundColor: getParameterColor(transform.parameter) }}
              >
                  ...
                  >
                    <td className="px-4 py-3">
                      <div
                        ...
                        style={{ backgroundColor: getParameterColor(result.parameter) }}
                      >
                        {result.parameter}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className="font-['Sniglet'] text-[12px] text-black dark:text-white">
                        {result.rawValue != null ? result.rawValue.toFixed(2) : '-'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className="font-['Sniglet'] text-[12px] text-black dark:text-white">
                        {result.computedScore != null ? result.computedScore.toFixed(2) : '-'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className="font-['Sniglet'] text-[12px] text-black dark:text-white">
                        {result.actualScore != null ? result.actualScore.toFixed(2) : '-'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span
                      ...
                      >
                        {result.difference != null ? `±${result.difference.toFixed(2)}` : '-'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      {result.error ? (
                        <Badge variant="destructive" className="font-['Sniglet'] text-[10px]">
                          <AlertCircle size={10} className="mr-1" />
                          {result.error}
                        </Badge>
                      ) : (
                        <Badge variant="default" className="font-['Sniglet'] text-[10px]">

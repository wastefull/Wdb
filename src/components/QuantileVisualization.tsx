...
export function QuantileVisualization({
  ...
  const color = highContrast ? SCORE_COLORS[scoreType].highContrast : SCORE_COLORS[scoreType].pastel;
...
function OverlapMode({ 
  scoreType, pracMean, theoMean, pracCI, theoCI, dotCount, width, height, 
  isHovered, reduceMotion, highContrast, darkMode, confidence 
}: any) {
...
function GapMode({ 
  scoreType, pracMean, theoMean, pracCI, theoCI, gap, dotCount, width, height,
  isHovered, reduceMotion, highContrast, darkMode, confidence 
}: any) {
  const centerY = height / 2;
...
function getAriaLabel(label: string, data: QuantileData, mode: VisualizationMode, gap: number): string {
  ...
  if (mode === 'overlap') {
    ...
    return `${label}: Practical ${pracScore}%, Theoretical ${theoScore}%. Gap of ${Math.round(gap * 100)} percentage points - science outpaces infrastructure. Confidence: ${confidence}. Click for details.`;
  }
}

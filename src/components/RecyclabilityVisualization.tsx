...
interface RecyclabilityVisualizationProps {
...
export function RecyclabilityVisualization({
  data,
  simplified = false,
  width = 300,
  height = 60,
  onClick,
  articleCount
}: RecyclabilityVisualizationProps) {
  ...
  if (simplified || !data.CR_practical_mean || !data.CR_theoretical_mean || 
      !data.CR_practical_CI95 || !data.CR_theoretical_CI95) {
    return (
      <SimpleBar
        score={(data.CR_practical_mean || 0) * 100}
        label="Recyclability"
        color="#e4e3ac"
        onClick={onClick}
  ...
  const pracMean = data.CR_practical_mean;
  const theoMean = data.CR_theoretical_mean;
  const pracCI = data.CR_practical_CI95;
  const theoCI = data.CR_theoretical_CI95;
  
  const gap = Math.abs(theoMean - pracMean);
  const overlap = (pracCI.upper >= theoCI.lower) && (theoCI.upper >= pracCI.lower);
  
  const mode: VisualizationMode = overlap ? 'overlap'
    : gap >= 0.10 ? 'gap'
    : 'near-overlap';
  
  // Responsive dot count
  const dotCount = isMobile ? 30 : width < 250 ? 60 : 100;
  
  return (
    <div className="flex flex-col gap-1 w-full">
      {/* Label and article count on top row */}
      <div className="flex justify-between items-center">
        <button
          ...
          aria-label={`View recyclability articles (${articleCount || 0} articles)`}
        >
          <span className="font-['Sniglet:Regular',_sans-serif] text-[11px] text-black dark:text-white">Recyclability Score</span>
      ...
      <div 
        ...
        role="button"
        tabIndex={0}
        aria-label={getAriaLabel(data, mode, gap)}
        onKeyDown={(e) => {
...
function SimpleBar({ 
  ...
  return (
    <div 
      ...
      role="button"
      tabIndex={0}
      aria-label={`${label}: ${score} out of 100${articleCount ? `, ${articleCount} article${articleCount !== 1 ? 's' : ''}` : ''}`}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          onClick?.();
    ...
    >
      <div className="flex items-center justify-between mb-1">
        <span className="font-['Sniglet:Regular',_sans-serif] text-[10px] text-black/70 dark:text-white/70">{label}</span>
        <div className="flex items-center gap-2">
          ...
          <span className="font-['Sniglet:Regular',_sans-serif] text-[10px] text-black/70 dark:text-white/70">{score}</span>
        </div>
      </div>
...
function OverlapMode({ 
  pracMean, theoMean, pracCI, theoCI, dotCount, width, height, 
  isHovered, reduceMotion, highContrast, confidence 
}: any) {
  ...
  const dots = Array.from({ length: dotCount }, (_, i) => {
    const value = overlapLower + (overlapUpper - overlapLower) * (i / (dotCount - 1));
    const x = value * width;
    return { x, value };
  });
  
  const dotColor = highContrast ? '#000000' : '#4C78A8';
  const haloColor = highContrast ? 'rgba(102, 102, 102, 0.7)' : 'rgba(160, 160, 160, 0.4)';
  ...
  return (
    <g>
      ...
      <ellipse
        ...
        ry={8}
        fill={highContrast ? 'rgba(51, 51, 102, 0.6)' : 'rgba(0, 102, 204, 0.3)'}
  ...
  return (
    <g>
      ...
      <ellipse
        ...
        rx={(pracCI.upper - pracCI.lower) * width / 2}
        ry={10}
        fill={highContrast ? 'rgba(102, 102, 102, 0.7)' : 'rgba(160, 160, 160, 0.4)'}
...
function GapMode({ 
  pracMean, theoMean, pracCI, theoCI, gap, dotCount, width, height,
  isHovered, reduceMotion, highContrast, confidence 
}: any) {
  const centerY = height / 2;
  ...
  return (
    <g>
      ...
      <defs>
        ...
        <pattern id="gapPattern" x="0" y="0" width="4" height="4" patternUnits="userSpaceOnUse">
...
// Axis component
function Axis({ width, height, highContrast }: { width: number; height: number; highContrast: boolean }) {
...
function TooltipContent({ 
  pracMean, theoMean, pracCI, theoCI, gap, mode, confidence 
}: any) {
  return (
      ...
      <div className="bg-white dark:bg-[#2a2825] border-[1.5px] border-[#211f1c] dark:border-white/20 rounded-md p-2 shadow-lg">
        <div className="font-['Sniglet:Regular',_sans-serif] text-[9px] text-black dark:text-white space-y-0.5">
          <div className="flex justify-between gap-3">
            ...
            <span>{Math.round(pracMean * 100)} ± {Math.round((pracCI.upper - pracCI.lower) * 50)}%</span>
          </div>
          <div className="flex justify-between gap-3">
            <span className="text-black/60 dark:text-white/60">Theoretical:</span>
            <span>{Math.round(theoMean * 100)} ± {Math.round((theoCI.upper - theoCI.lower) * 50)}%</span>
          </div>
          <div className="flex justify-between gap-3">
...
function getAriaLabel(data: RecyclabilityData, mode: VisualizationMode, gap: number): string {
  const pracScore = Math.round((data.CR_practical_mean || 0) * 100);
  const theoScore = Math.round((data.CR_theoretical_mean || 0) * 100);
  const confidence = data.confidence_level || 'Unknown';
  
  if (mode === 'overlap') {
    return `Recyclability: Practical and theoretical scores overlap around ${pracScore}%. Confidence: ${confidence}. Click for details.`;
  } else if (mode === 'near-overlap') {
    return `Recyclability: Practical ${pracScore}%, Theoretical ${theoScore}%. Minor difference. Confidence: ${confidence}. Click for details.`;
  } else {
    return `Recyclability: Practical ${pracScore}%, Theoretical ${theoScore}%. Gap of ${Math.round(gap * 100)} percentage points - science outpaces infrastructure. Confidence: ${confidence}. Click for details.`;
  }
}

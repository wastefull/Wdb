...
/**
 * Suggest a confidence level from citation coverage.
 *
 * The UI guidance in SourcesTab is source-count based, so we keep the
 * thresholds aligned with that behavior to avoid contradictory warnings.
 */
export function getSuggestedConfidenceLevel(
  ...
  // Keep parameter for API compatibility with previous helper signature.
  void totalWeight;
  return "Low";
}
...
export function autoAssignParameters(tags: string[]): string[] {
  ...
  if (
    tags.some((t) => ["infrastructure", "maturity", "facilities"].includes(t))
  ) {
  ...
  if (
    tags.some((t) => ["upgrade", "upgradability", "modularity"].includes(t))
  ) {

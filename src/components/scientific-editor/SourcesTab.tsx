...
export function SourcesTab({ material, sources, onSourcesChange, onParameterChange }: SourcesTabProps) {
      ...
      {levelHierarchy[currentLevel] > levelHierarchy[suggestedLevel] && (
        <Alert className="bg-amber-50 dark:bg-amber-900/20 border-amber-300 dark:border-amber-700">
          ...
          <AlertDescription className="text-[11px] text-amber-800 dark:text-amber-200">
            <strong>Confidence Level Mismatch:</strong> Current level is "{currentLevel}" but you only have {sources.length} source(s). 
      ...
      <Card className="p-4 bg-[#faf9f6] dark:bg-[#1a1918] border-[#211f1c] dark:border-white/20">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Select
              value={currentLevel}
              onValueChange={(value: 'High' | 'Medium' | 'Low') => onParameterChange('confidence_level', value)}
            >
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => onParameterChange('whitepaper_version', e.target.value)}
                ...
                <div className="flex-1">
                  ...
                  {source.parameters && source.parameters.length > 0 && (
                    <div className="mt-2 text-[9px] text-black/60 dark:text-white/60">
                      ...
                      {source.parameters.map(param => PARAMETER_NAMES[param] || param).join(', ')}
                    </div>
                  )}
                </div>
                <Button
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-2 flex-wrap">
                              {source.year && <Badge variant="outline" className="text-[8px]">{source.year}</Badge>}
                              <Badge variant="outline" className="text-[8px]">{source.type}</Badge>
                              {source.tags && source.tags.slice(0, 3).map(tag => (

 ...
 */
export async function createEvidencePoint(
  input: CreateEvidencePointInput,
 ...
 */
export async function getEvidencePoint(
  id: string,
...
export async function getEvidencePointsByMaterial(
  ...
  evidence = evidence.filter((e): e is EvidencePoint => e !== null);
 ...
 */
export async function getEvidencePointsBySource(
  sourceRef: string,
): Promise<EvidencePoint[]> {
  ...
  return evidence.filter((e): e is EvidencePoint => e !== null);
}
 ...
 */
export async function canDeleteSource(sourceRef: string): Promise<{
  canDelete: boolean;
...
export async function getEvidenceStatsByMaterial(
  ...
  byDimension.forEach((eps, dimension) => {
    ...
    const avgSourceWeight =
      eps.reduce((sum, ep) => sum + ep.source_weight, 0) / eps.length;

    const validatedCount = eps.filter(
      (ep) => ep.validation_status === "validated",
    ).length;
    const pendingCount = eps.filter(
      (ep) => ep.validation_status === "pending",
    ).length;
    const flaggedCount = eps.filter(
      (ep) => ep.validation_status === "flagged",
    ).length;

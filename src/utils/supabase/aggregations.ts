...
function computeWeightedMean(evidencePoints: evidence.EvidencePoint[]): {
  ...
  const values = evidencePoints.map(
    (ep) => ep.transformed_value ?? ep.raw_value,
  );
  const weights = evidencePoints.map((ep) => ep.source_weight);

  // Weighted mean
  const sumWeightedValues = values.reduce(
...
export async function createAggregation(
  ...
  // Filter out nulls
  const validEvidence = evidencePoints.filter(
    (ep): ep is evidence.EvidencePoint => ep !== null,
  );
  ...
  // Compute aggregation
  const { mean, se, ci95_lower, ci95_upper } =
    computeWeightedMean(validEvidence);
  ...
  if (existingCurrentId) {
    ...
    const existingCurrent = (await kv.get(
      `aggregation:${existingCurrentId}`,
    )) as ParameterAggregation | null;
    if (existingCurrent) {
...
export async function getAggregation(
  id: string,
): Promise<ParameterAggregation | null> {
...
export async function getCurrentAggregationsByMaterial(
  ...
  return aggregations.filter((a): a is ParameterAggregation => a !== null);
}
...
export async function getAggregationHistory(
  ...
  aggregations = aggregations.filter(
    (a): a is ParameterAggregation => a !== null,
  );
...
export async function getAggregationStats(
  ...
  const currentAggregations =
    await getCurrentAggregationsByMaterial(materialId);

  const totalMius = currentAggregations.reduce(
    (sum, agg) => sum + agg.n_mius,
    0,
  );
  ...
  if (currentAggregations.length > 0) {
    const sorted = [...currentAggregations].sort(
      (a, b) =>
...
export async function getAggregationCoverageMatrix(
  ...
  if (materialId) {
    ...
    aggregations = (await Promise.all(aggregationPromises)).filter(
      (a): a is ParameterAggregation => a !== null,
    );
  }
  ...
  return aggregations.map((agg) => {
    ...
    const ci_width =
      agg.ci95_lower !== null && agg.ci95_upper !== null

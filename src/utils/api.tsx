...
// onBehalfOf: Admin-only parameter to attribute the material to another user
export async function saveMaterial(
  material: Material,
  options?: { onBehalfOf?: string },
): Promise<Material> {
  const payload = options?.onBehalfOf
    ? { ...material, on_behalf_of: options.onBehalfOf }
    : material;
...
export async function seedAuditLogFromKV(): Promise<{
  success: boolean;
  inserted: number;
  skipped: number;
  errors: number;
  ...
  return await apiCall("/admin/audit/seed-from-kv", { method: "POST" });
}

export async function seedRolesFromKV(): Promise<{
  success: boolean;
  updated: number;
  skipped: number;
  errors: number;
  ...
  return await apiCall("/admin/roles/seed-from-kv", { method: "POST" });
}

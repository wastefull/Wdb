-- Stage 7: reviewed triage stores human-readable identifiers while newer
-- admin-created materials may use UUID legacy IDs/slugs. Resolve conservative
-- normalized names and aliases in addition to exact identifiers.
...
BEGIN
  ...
  LOOP
    ...
    WHERE lower(v_candidate.material_identifier) IN (
      lower(m.id::TEXT),
      lower(COALESCE(m.legacy_kv_id, '')),
      lower(COALESCE(m.slug, '')),
      lower(m.name)
    )
    OR regexp_replace(lower(v_candidate.material_identifier), '[^a-z0-9]+', '', 'g') =
       regexp_replace(lower(m.name), '[^a-z0-9]+', '', 'g')
    OR EXISTS (
      SELECT 1
      FROM unnest(COALESCE(m.aliases, '{}'::TEXT[])) alias(value)
      WHERE regexp_replace(lower(v_candidate.material_identifier), '[^a-z0-9]+', '', 'g') =
            regexp_replace(lower(alias.value), '[^a-z0-9]+', '', 'g')
    );

-- Stage 7: promote explicitly reviewed video topic tags into governed tags.

BEGIN;

INSERT INTO public.tags (slug, label, tag_type, description, active)
VALUES (
  '3d_printing',
  '3D printing',
  'topic',
  'Substantive coverage of additive manufacturing, feedstocks, print waste, repair, reuse, or recovery.',
  TRUE
)
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    lower(btrim(topic_slug)) AS topic_slug,
    tag.id AS tag_id,
  ...
  CROSS JOIN LATERAL unnest(vii.reviewed_topic_tags) AS topic_slug
  LEFT JOIN public.tags tag
    ON tag.slug = lower(btrim(topic_slug))
   AND tag.tag_type = 'topic'
   AND tag.active = TRUE
  LEFT JOIN public.entity_tags existing
    ON existing.entity_id = binding.entity_id
   AND existing.tag_id = tag.id
  WHERE vii.review_status = 'reviewed'
    AND vii.video_id IS NOT NULL
    AND NULLIF(btrim(topic_slug), '') IS NOT NULL;

  SELECT count(*), count(*) FILTER (WHERE tag_id IS NOT NULL),
    count(*) FILTER (WHERE tag_id IS NULL),
    count(*) FILTER (WHERE already_tagged),
    count(*) FILTER (WHERE tag_id IS NOT NULL AND NOT already_tagged)
  INTO v_candidate_count, v_resolved_count, v_unresolved_count,
    v_existing_count, v_creatable_count
  FROM reviewed_video_topic_candidates;

  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'import_item_id', import_item_id,
    'video_entity_id', video_entity_id,
    'topic_slug', topic_slug
  ) ORDER BY topic_slug, import_item_id), '[]'::JSONB)
  INTO v_unresolved
  FROM reviewed_video_topic_candidates

-- Stage 7: transactional human review for staged video candidates.
--
-- This function updates private triage decisions and aggregate batch status.
-- It cannot create videos, entities, mappings, tags, or editorial leads.
...
BEGIN
  ...
  IF v_batch.status IN ('applying', 'completed', 'failed', 'archived') THEN
...
REVOKE ALL ON FUNCTION public.review_video_triage_item(
  UUID,
  UUID,
  TEXT,
  TEXT[],
...
GRANT EXECUTE ON FUNCTION public.review_video_triage_item(
  UUID,
  UUID,
  TEXT,
  TEXT[],

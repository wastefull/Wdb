-- Stage 7: treat permalink-style slugs (derived from material names)
-- as exact matches before fuzzy/normalized matching for reviewed triage links.
...
BEGIN
  ...
  LOOP
    ...
    -- Includes permalink-style slugs derived from material names.
    ...
    WHERE v_identifier_lower IN (
      ...
      lower(m.name),
      btrim(regexp_replace(regexp_replace(lower(m.name), '[^a-z0-9]+', '-', 'g'), '(^-+|-+$)', '', 'g'), '-')
    );
